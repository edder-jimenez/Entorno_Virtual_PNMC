...
const AxisSection = ({ eje, onNavigateComponent }) => {
  ...
  const titleOverride = getWebText(`eje${eje.id}_title`) || eje.title;
  const explainOverride = [
    getWebText(`eje${eje.id}_desc1`) || eje.axisExplain[0],
    getWebText(`eje${eje.id}_desc2`) || eje.axisExplain[1]
  ];
  const purposeOverride = getWebText(`eje${eje.id}_purpose`) || eje.purpose;
      ...
      <ContentWrapper className="!py-0">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-20 items-center mb-12">
          <div className="lg:col-span-5 space-y-6">
            ...
            <h3 className="font-alternate text-4xl lg:text-5xl text-[#291242] font-bold uppercase leading-[1.1] tracking-tight text-balance">
              {titleOverride}
            </h3>
            <div className="space-y-6">
              {explainOverride.map((paragraph, idx) => (
          ...
          <div className="lg:col-span-7">
            <div className="relative rounded-[2.5rem] overflow-hidden bg-slate-100 shadow-2xl group border-8 border-slate-50 flex flex-col">
              <div className="relative aspect-video w-full overflow-hidden">
                <img src={eje.videoImg} className="w-full h-full object-cover grayscale-[40%] brightness-95 opacity-80 group-hover:grayscale-0 group-hover:brightness-100 group-hover:opacity-100 transition-all duration-1000" alt="" />
                <div className="absolute inset-0 bg-[#291242]/20 group-hover:opacity-0 transition-opacity duration-1000 pointer-events-none"></div>
                <div className="absolute inset-0 flex items-center justify-center">
                  <button className="w-20 h-20 bg-[#8BF784] text-[#291242] rounded-full flex items-center justify-center shadow-2xl hover:scale-110 active:scale-95 transition-all cursor-pointer">
        ...
        <div className="bg-slate-50/70 p-4 md:p-8 rounded-[2.5rem] border border-slate-100">
          ...
          <div className="flex flex-col gap-3">
            {eje.components.map((comp, index) => {
              ...
              const compTitleOverride = getWebText(`eje${eje.id}_c${index + 1}_title`) || comp.name;
              const compDetailsOverride = getWebText(`eje${eje.id}_c${index + 1}_desc`) || comp.details;
                ...
                <div key={comp.id} onClick={() => setExpandedIndex(index)} className={`relative transition-all duration-700 ease-[cubic-bezier(0.4,0,0.2,1)] cursor-pointer overflow-hidden rounded-[1.8rem] border ${isExpanded ? 'bg-white border-[#8BF784] shadow-lg' : 'bg-slate-200/50 border-transparent hover:bg-slate-200'}`}>
                  ...
                  <div className={`overflow-hidden transition-all duration-700 ${isExpanded ? 'max-h-[420px] opacity-100' : 'max-h-0 opacity-0 pointer-events-none'}`}>
                    <div className="p-8 md:p-10 flex flex-col">
                      ...
                      <p className="font-nunito text-slate-600 text-sm leading-relaxed font-light line-clamp-3">{compDetailsOverride}</p>
                      <button
                        onClick={(event) => {
                          event.stopPropagation();
                          onNavigateComponent(comp.id);
                        }}
                        className="mt-8 flex items-center gap-2 text-[0.6rem] font-bold text-[#291242] uppercase font-alternate tracking-widest group/btn transition-all duration-300 hover:gap-4 hover:text-[#00DA5E] cursor-pointer"
                      >
                        Explorar componente
...
const EjesPage = ({ onBack, onNavigateComponent }) => {
  return (
    <div className="bg-white min-h-screen pb-20 text-left font-nunito">
