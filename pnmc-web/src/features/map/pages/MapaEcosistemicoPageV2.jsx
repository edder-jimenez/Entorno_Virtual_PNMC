...
import { MapV2Toolbar } from '../components/map-v2/MapV2Toolbar.jsx';
import { MapV2OverlayPanel } from '../components/map-v2/MapV2OverlayPanel.jsx';
import { MAP_V2_LAYERS_CONFIG, MAP_V2_PANEL_IDS } from '../config/mapLayersConfig.js';
import {
  ECOSYSTEM_LAYERS,
...
const MapaEcosistemicoPageV2 = ({ onBack, navigationRequest, onOpenParticipation }) => {
  const immersiveMapMode = true;
  const [activeCategory, setActiveCategory] = useState('General');
  const [activeView, setActiveView] = useState('map');
  const [activePanel, setActivePanel] = useState(null);
  const [detailPanelTab, setDetailPanelTab] = useState('resumen');
  ...
  const mapViewportHeight = immersiveMapMode
    ? 'calc(100vh - 136px)'
    : activeView === 'map'
    ? 'clamp(820px, calc(100vh - 136px), 1700px)'
    : 'clamp(640px, 78vh, 1080px)';
  ...
  const colombiaBounds = useMemo(() => {
    ...
    if (immersiveMapMode) {
      const nationalFeatures = geoData.features.filter(
        (feature) => getFeatureDepartmentNormalizedName(feature) !== 'SAN ANDRES Y PROVIDENCIA'
      );
      const immersiveCollection = {
        ...geoData,
        features: nationalFeatures.length > 0 ? nationalFeatures : geoData.features,
      };
      return L.geoJSON(immersiveCollection).getBounds();
    }
  ...
  }, [geoData, immersiveMapMode, selectedDept]);
  ...
  const activeLayerV2Config = useMemo(
    () => MAP_V2_LAYERS_CONFIG.find((layer) => layer.layerKey === activeCategory) || MAP_V2_LAYERS_CONFIG[0],
    [activeCategory]
  );
  ...
  }, [activeCategory, immersiveMapMode]);
  ...
  const handleExportLayerCsv = useCallback(() => {
    ...
    const layerSlug = String(activeLayerV2Config?.id || 'layer');
    link.href = url;
    link.setAttribute('download', `mapa-ecosistemico-v2-${layerSlug}.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);
  }, [activeCategory, activeDepartmentCounts, activeLayerV2Config?.id]);
  ...
  useEffect(() => {
    if (!immersiveMapMode) return;
    if (activeView !== 'map') {
      setActiveView('map');
    }
  }, [activeView, immersiveMapMode]);
  useEffect(() => {
    if (!immersiveMapMode) return;
    const tutorialSeenKey = 'pnmc-map-v2-tutorial-seen';
    const alreadySeen = window.sessionStorage.getItem(tutorialSeenKey) === '1';
    if (!alreadySeen) {
      setActivePanel(MAP_V2_PANEL_IDS.tutorial);
      window.sessionStorage.setItem(tutorialSeenKey, '1');
    }
  }, [immersiveMapMode]);
  useEffect(() => {
    if (!immersiveMapMode) return undefined;
  ...
  return (
    <div className="h-screen text-left relative overflow-hidden bg-[radial-gradient(circle_at_20%_0%,#d7e2f3_0%,#eef1f5_42%,#e7ebf1_100%)]">
      {!immersiveMapMode && (
        <PageHero 
      ...
      <ContentWrapper
        fullBleed={immersiveMapMode}
        innerClassName={immersiveMapMode ? 'w-full h-full px-0' : ''}
        className={immersiveMapMode ? '!pt-[86px] !pb-0 !h-screen !scroll-mt-0 overflow-hidden' : '!py-8'}
      >
        {!immersiveMapMode && (
          <LayerStatusStrip
        ...
        <div id="mapa-workspace" ref={mapWorkspaceRef} className={`${immersiveMapMode ? 'grid h-full grid-cols-1 gap-0 items-start' : 'grid grid-cols-1 lg:grid-cols-[minmax(0,1.3fr)_minmax(0,4fr)] gap-3 items-start scroll-mt-28'}`}>
          <aside className={`${immersiveMapMode ? 'hidden' : 'space-y-3 sticky top-28'}`}>
          ...
          <div className={immersiveMapMode ? 'h-full space-y-0' : 'space-y-3'}>
            <div className={`bg-white border border-slate-200 ${immersiveMapMode ? 'h-full rounded-none p-2 md:p-3 border-x-0 border-y-0 bg-transparent' : 'rounded-[3rem] p-10'} ${activeView === 'map' ? '' : activeView === 'grid' ? 'min-h-[1080px]' : 'min-h-[980px]'}`}>
              {!immersiveMapMode && (
              <div className={`flex flex-col md:flex-row md:items-center justify-between items-start gap-4 ${immersiveMapMode ? 'mb-2' : 'mb-10'}`}>
                <div>
                  <h3 className={`font-alternate font-bold uppercase text-[#291242] leading-none ${immersiveMapMode ? 'text-lg md:text-xl' : 'text-2xl'}`}>Ecosistema Musical · Geovisor V2</h3>
                  <div className="mt-2 flex flex-wrap items-center gap-2">
                    <span className="rounded-full border border-slate-200 bg-white px-3 py-1 text-[0.52rem] font-bold uppercase tracking-[0.12em] text-slate-500">
                      Capa: {activeLayerConfig.shortLabel}
                    </span>
                    <span className="rounded-full border border-slate-200 bg-white px-3 py-1 text-[0.52rem] font-bold uppercase tracking-[0.12em] text-slate-500">
                      Territorio: {selectedDepartmentDisplayName}
                    </span>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  {immersiveMapMode && (
                    <button
                      type="button"
                      onClick={onBack}
                      className="h-11 px-4 rounded-xl border border-slate-200 bg-white text-[0.56rem] font-bold uppercase tracking-[0.12em] text-[#291242] hover:border-slate-300 transition-all"
                    >
                      Volver
                    </button>
                  )}
              ...
              {activeView === 'map' ? (
                <div className={immersiveMapMode ? 'space-y-0' : 'space-y-8'}>
                  <div>
                      <div className={`w-full mx-auto bg-[#dfe5ed] relative overflow-hidden ring-1 ring-slate-300/60 ${immersiveMapMode ? 'rounded-[1.2rem] xl:pr-[392px]' : 'rounded-[2.8rem]'}`} style={{ height: mapViewportHeight }}>
                      {isLoading ? (
                        ...
                        <MapContainer
                        className={immersiveMapMode ? 'map-v2-canvas' : undefined}
                        ...
                        maxBoundsViscosity={immersiveMapMode ? 0.08 : 0.35}
                        preferCanvas={true}
                        style={{ height: '100%', width: '100%', background: 'transparent', zIndex: 1 }}
                        scrollWheelZoom={true}
                        wheelDebounceTime={14}
                        wheelPxPerZoomLevel={immersiveMapMode ? 110 : 90}
                        dragging={true}
                        ...
                        >
                          ...
                          {!immersiveMapMode && <MapTrackpadGestureHandler />}
                      ...
                      {immersiveMapMode && (
                        <>
                          <section className="pointer-events-none absolute left-4 right-4 top-4 z-[1185] flex items-start justify-between gap-3">
                            <div className="pointer-events-auto rounded-2xl border border-slate-200 bg-white/92 px-3 py-2 shadow-lg backdrop-blur-sm">
                              <h3 className="font-alternate text-[0.92rem] font-bold uppercase leading-none text-[#291242]">Geovisor Ecosistema Musical</h3>
                              <div className="mt-1.5 flex flex-wrap items-center gap-1.5">
                                <span className="rounded-full border border-slate-200 bg-white px-2.5 py-1 text-[0.48rem] font-bold uppercase tracking-[0.12em] text-slate-500">
                                  Capa: {activeLayerConfig.shortLabel}
                                </span>
                                <span className="rounded-full border border-slate-200 bg-white px-2.5 py-1 text-[0.48rem] font-bold uppercase tracking-[0.12em] text-slate-500">
                                  Territorio: {selectedDepartmentDisplayName}
                            ...
                            <div className="pointer-events-auto flex items-center gap-2">
                              {selectedDept !== 'Nacional' && (
                                <button
                                  ...
                                  className="h-10 rounded-xl border border-slate-200 bg-white/95 px-3 text-[0.52rem] font-bold uppercase tracking-[0.12em] text-[#291242] shadow-sm hover:border-slate-300"
                                >
                                  Vista general
                                </button>
                              )}
                          ...
                          {activePanel === MAP_V2_PANEL_IDS.layers && (
                            <MapV2OverlayPanel
                              title="Capas de información"
                              subtitle="Activa una capa para enfocar la lectura territorial. La vista general integra las tres capas principales."
                              onClose={() => setActivePanel(null)}
                            >
                              <div className="space-y-2">
                                {MAP_V2_LAYERS_CONFIG.map((layer) => (
                                  <button
                                    key={layer.id}
                                    type="button"
                                  ...
                                  >
                                    <div className="flex items-center justify-between gap-3">
                                      <p className="text-[0.65rem] font-bold uppercase tracking-[0.14em]">{layer.label}</p>
                                      <span
                                        className="h-2.5 w-2.5 rounded-full"
                                        style={{ backgroundColor: layer.color }}
                                      />
                                    </div>
                                    <p className={`mt-1.5 text-[0.62rem] leading-relaxed ${activeCategory === layer.layerKey ? 'text-slate-200' : 'text-slate-500'}`}>
                                      {layer.description}
                                    </p>
                                  </button>
                                ))}
                          ...
                          {activePanel === MAP_V2_PANEL_IDS.filters && (
                            <MapV2OverlayPanel
                              title="Filtros"
                              subtitle="Refina la exploración sin salir del mapa."
                              onClose={() => setActivePanel(null)}
                            >
                              <div className="space-y-4">
                                <div>
                                  <label className="text-[0.5rem] font-bold uppercase tracking-[0.16em] text-slate-400">Territorio</label>
                                  <select
                                    value={selectedDept}
                                    onChange={(e) => setselectedDept(e.target.value)}
                                    className="mt-1.5 h-10 w-full rounded-xl border border-slate-200 bg-white px-3 text-[0.64rem] font-alternate uppercase text-[#291242] outline-none focus:border-[#00DA5E]"
                                  >
                                ...
                                <div className="flex gap-2">
                                  <button
                                    type="button"
                                    onClick={handleReturnToNationalView}
                                    className="h-10 flex-1 rounded-xl border border-slate-200 bg-white px-3 text-[0.58rem] font-bold uppercase tracking-[0.12em] text-slate-600 hover:text-[#291242]"
                                  >
                                    Vista nacional
                                  </button>
                                  <button
                                    ...
                                    className="h-10 flex-1 rounded-xl bg-[#291242] px-3 text-[0.58rem] font-bold uppercase tracking-[0.12em] text-white hover:bg-[#6100D7]"
                                  >
                                    Actualizar
                                  </button>
                                </div>
                          ...
                          {activePanel === MAP_V2_PANEL_IDS.insights && (
                            <MapV2OverlayPanel
                              title="Resumen territorial"
                              subtitle={activeInfoNote}
                              onClose={() => setActivePanel(null)}
                            >
                              <div className="space-y-2">
                                {technicalConsultationSections.slice(0, 5).map((item) => (
                                  <div key={item.label} className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5">
                                    <p className="text-[0.5rem] font-bold uppercase tracking-[0.14em] text-slate-400">{item.label}</p>
                                    <p className="mt-1 font-alternate text-[0.9rem] uppercase text-[#291242]">{item.value}</p>
                                    <p className="text-[0.56rem] text-slate-500 mt-1">{item.note}</p>
                                  </div>
                                ))}
                              </div>
                            </MapV2OverlayPanel>
                          ...
                          {activePanel === MAP_V2_PANEL_IDS.tutorial && (
                            <div className="absolute inset-0 z-[1210] flex items-center justify-center bg-[#0f172a]/35 backdrop-blur-[1px] p-4">
                              <section className="w-[min(94vw,560px)] rounded-3xl border border-slate-200 bg-white p-6 shadow-2xl">
                                <header className="flex items-start justify-between gap-4">
                                  <div>
                                    <p className="text-[0.52rem] font-bold uppercase tracking-[0.16em] text-slate-400">Bienvenido al Geovisor</p>
                                    <h3 className="mt-1.5 font-alternate text-[1.05rem] font-bold uppercase text-[#291242]">Tutorial de uso</h3>
                                    <p className="mt-2 text-[0.68rem] text-slate-500">Flujo recomendado para navegar el módulo.</p>
                                  </div>
                                  <button
                                    type="button"
                                    onClick={() => setActivePanel(null)}
                                    className="rounded-full border border-slate-200 px-3 py-1.5 text-[0.56rem] font-bold uppercase tracking-[0.12em] text-slate-500 hover:text-[#291242]"
                                  >
                                    Cerrar
                                  </button>
                                </header>
                                <ol className="mt-4 list-decimal space-y-2.5 pl-5 text-[0.68rem] text-slate-600">
                                  <li>Elige una capa en el panel de Capas o usa Vista general.</li>
                                  <li>Usa Filtros para enfocarte en un departamento.</li>
                                  <li>Pasa el cursor sobre el territorio para activar la lectura territorial.</li>
                                  <li>Haz clic sobre un departamento para enfocar indicadores y usar “Vista general”.</li>
                                  <li>Exporta o imprime desde la barra lateral cuando necesites reporte.</li>
                                </ol>
                                <div className="mt-5 flex items-center justify-end gap-2">
                                  <button
                                    type="button"
                                    onClick={() => setActivePanel(MAP_V2_PANEL_IDS.layers)}
                                    className="h-10 rounded-xl border border-slate-200 px-4 text-[0.58rem] font-bold uppercase tracking-[0.12em] text-slate-600 hover:text-[#291242]"
                                  >
                                    Ir a capas
                                  </button>
                                  <button
                                    type="button"
                                    onClick={() => setActivePanel(null)}
                                    className="h-10 rounded-xl bg-[#291242] px-4 text-[0.58rem] font-bold uppercase tracking-[0.12em] text-white hover:bg-[#6100D7]"
                                  >
                                    Entendido
                                  </button>
                                </div>
                          ...
                          {activePanel === MAP_V2_PANEL_IDS.export && (
                            <MapV2OverlayPanel
                              title="Exportar e imprimir"
                              subtitle="Acciones rápidas para reporte y socialización."
                              onClose={() => setActivePanel(null)}
                            >
                              <div className="space-y-2">
                                <button
                                  ...
                                  className="w-full rounded-xl bg-[#291242] px-4 py-3 text-[0.58rem] font-bold uppercase tracking-[0.12em] text-white hover:bg-[#6100D7]"
                                >
                                  Exportar CSV de capa activa
                                </button>
                                <button
                                  type="button"
                                  onClick={handlePrintMap}
                                  className="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-[0.58rem] font-bold uppercase tracking-[0.12em] text-slate-600 hover:text-[#291242]"
                                >
                          ...
                          {immersiveMapMode && (
                            <aside className="pointer-events-auto absolute bottom-3 right-3 top-3 z-[1188] hidden w-[380px] rounded-2xl border border-slate-200 bg-white/95 shadow-xl backdrop-blur-sm xl:flex xl:flex-col">
                              <div className="border-b border-slate-100 px-4 py-3">
                                <div className="flex items-center justify-between gap-3">
                                  <div>
                                    <p className="text-[0.48rem] font-bold uppercase tracking-[0.14em] text-slate-400">Consulta territorial</p>
                                    <h4 className="mt-1 font-alternate text-[0.95rem] font-bold uppercase text-[#291242]">{selectedDepartmentDisplayName}</h4>
                                  </div>
                                  {selectedDept !== 'Nacional' && (
                                    <button
                                      type="button"
                                      onClick={handleReturnToNationalView}
                                      className="rounded-lg border border-slate-200 px-2.5 py-1 text-[0.5rem] font-bold uppercase tracking-[0.12em] text-slate-500 hover:text-[#291242]"
                                    >
                                      Vista nacional
                                    </button>
                                  )}
                                ...
                                <div className="mt-3 grid grid-cols-3 gap-2">
                                  {[
                                    { label: 'Festivales', value: selectedDept === 'Nacional' ? festivalAnalytics.totalRecords : selectedFestivalRecords.length },
                                    { label: 'Escuelas', value: selectedDept === 'Nacional' ? schoolAnalytics.totalRecords : selectedSchoolRecords.length },
                                    { label: 'Mercados', value: selectedDept === 'Nacional' ? marketAnalytics.totalRecords : selectedMarketRecords.length },
                                  ].map((item) => (
                                    <div key={item.label} className="rounded-lg bg-slate-50 px-2 py-2">
                                      <p className="text-[0.46rem] font-bold uppercase tracking-[0.1em] text-slate-400">{item.label}</p>
                                      <p className="mt-1 font-alternate text-[0.9rem] font-bold text-[#291242]">{formatMetricValue(item.value)}</p>
                                    </div>
                                  ))}
                                </div>
                                <div className="mt-3 grid grid-cols-3 gap-1.5 rounded-xl bg-slate-100 p-1">
                                  {[
                                    ...
                                    <button
                                      ...
                                      className={`h-8 rounded-lg text-[0.5rem] font-bold uppercase tracking-[0.12em] transition ${
                                        detailPanelTab === tab.key
                                ...
                                <div className="mt-2 rounded-xl border border-slate-200 bg-slate-50 px-2.5 py-2">
                                  <div className="flex items-center justify-between gap-3">
                                    <p className="text-[0.45rem] font-bold uppercase tracking-[0.12em] text-slate-400">Lectura territorial</p>
                                    <span className="text-[0.44rem] font-bold uppercase tracking-[0.1em] text-slate-500">En vivo</span>
                                  </div>
                                  <p className="mt-0.5 font-alternate text-[0.74rem] uppercase text-[#291242]">{selectedDepartmentDisplayName}</p>
                                  {hoveredDepartmentCard ? (
                                    <div
                                      className="territorial-hover-card mt-1.5 max-h-[148px] overflow-y-auto rounded-xl border border-slate-200 bg-white"
                                      dangerouslySetInnerHTML={{
                                        __html: sanitizeHtml(activePopupMarkupBuilder({
                                          deptName: hoveredDepartmentCard.deptName,
                                          stats: hoveredDepartmentCard.stats,
                                          embedded: true,
                                        })),
                                      }}
                                    />
                                  ) : (
                                    <p className="mt-1.5 rounded-lg border border-dashed border-slate-200 bg-white px-2 py-1.5 text-[0.52rem] leading-relaxed text-slate-500">
                                      Pasa el cursor por un departamento para ver una ficha rápida con indicadores.
                                    </p>
                                  )}
                                </div>
                              </div>
                              <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
                                {detailPanelTab === 'resumen' && (
                                  <div className="space-y-2">
                                    {(selectedDept === 'Nacional'
                                      ? technicalConsultationSections.slice(0, 8)
                                      : [
                                        { label: 'Festivales', value: formatMetricValue(focusedDepartmentStats?.festivalCount || 0), note: 'Registros visibles en el territorio.' },
                                        { label: 'Escuelas', value: formatMetricValue(focusedDepartmentStats?.schoolCount || 0), note: 'Procesos formativos reportados.' },
                                        { label: 'Mercados', value: formatMetricValue(focusedDepartmentStats?.marketCount || 0), note: 'Nodos de circulación activos.' },
                                        { label: 'Estudiantes', value: formatMetricValue(focusedDepartmentStats?.totalStudents || 0), note: 'Capacidad formativa declarada.' },
                                        { label: 'Docentes', value: formatMetricValue(focusedDepartmentStats?.totalTeachers || 0), note: 'Talento pedagógico asociado.' },
                                        { label: 'Proyectos (mercados)', value: formatMetricValue(focusedDepartmentStats?.totalMarketProjects || 0), note: 'Promoción de circulación musical.' },
                                      ]).map((item) => (
                                        <div key={item.label} className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5">
                                          <p className="text-[0.48rem] font-bold uppercase tracking-[0.12em] text-slate-400">{item.label}</p>
                                          <p className="mt-1 font-alternate text-[0.92rem] uppercase text-[#291242]">{item.value}</p>
                                          <p className="mt-1 text-[0.56rem] text-slate-500">{item.note}</p>
                                        </div>
                                      ))}
                                  </div>
                                )}

                                {detailPanelTab === 'infografia' && (
                                  <div className="space-y-2">
                                    {(technicalSignalCards || []).map((card) => (
                                      <div key={card.label} className="rounded-xl border border-slate-200 bg-white px-3 py-3">
                                        <p className="text-[0.48rem] font-bold uppercase tracking-[0.12em] text-slate-400">{card.label}</p>
                                        <p className="mt-1 font-alternate text-[0.96rem] text-[#291242]">{formatMetricValue(card.value)}</p>
                                        <p className="mt-1 text-[0.56rem] text-slate-500">{card.note}</p>
                                      </div>
                                    ))}
                                  </div>
                                )}

                                {detailPanelTab === 'datos' && (
                                  <div className="space-y-3">
                                    {selectedDept === 'Nacional' ? (
                                      <div className="space-y-2">
                                        <p className="text-[0.56rem] font-bold uppercase tracking-[0.12em] text-slate-400">Departamentos destacados</p>
                                        {activeAnalytics.topDepartments.slice(0, 10).map((item, index) => (
                                          <button
                                            key={item.name}
                                            type="button"
                                            onClick={() => handleDepartmentDrilldown(item.name, activeCategory)}
                                            className="flex w-full items-center justify-between rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-left hover:border-slate-300"
                                          >
                                            <span className="text-[0.62rem] font-bold uppercase text-[#291242]">{index + 1}. {getDepartmentDisplayName(item.name)}</span>
                                            <span className="text-[0.58rem] font-bold text-slate-500">{formatMetricValue(item.count)}</span>
                                          </button>
                                        ))}
                                      </div>
                                    ) : (
                                      <div className="space-y-3">
                                        <div>
                                          <p className="text-[0.56rem] font-bold uppercase tracking-[0.12em] text-slate-400">Registros visibles</p>
                                          <div className="mt-2 space-y-2">
                                            {[
                                              ...selectedFestivalRecords.slice(0, 4).map((item) => ({ type: 'Festival', name: item.name || 'Sin nombre' })),
                                              ...selectedSchoolRecords.slice(0, 4).map((item) => ({ type: 'Escuela', name: item.name || 'Sin nombre' })),
                                              ...selectedMarketRecords.slice(0, 4).map((item) => ({ type: 'Mercado', name: item.name || 'Sin nombre' })),
                                            ].map((item, index) => (
                                              <div key={`${item.type}-${index}`} className="rounded-xl border border-slate-200 bg-white px-3 py-2">
                                                <p className="text-[0.48rem] font-bold uppercase tracking-[0.12em] text-slate-400">{item.type}</p>
                                                <p className="mt-1 text-[0.66rem] font-medium text-[#291242] leading-snug">{item.name}</p>
                                              </div>
                                            ))}
                                          </div>
                                        </div>
                      ...
                      {!immersiveMapMode && (isGeneralLayer ? (
                      ...
                      {immersiveMapMode ? (
                        <div className="pointer-events-none absolute bottom-5 left-5 z-[1180] hidden max-w-[280px] rounded-2xl border border-slate-200 bg-white/95 p-3 shadow-lg backdrop-blur-sm md:block">
                          <p className="text-[0.5rem] font-bold uppercase tracking-[0.14em] text-slate-400">Leyenda</p>
                          <div className="mt-2 grid grid-cols-1 gap-1.5">
                            {activeLegendItems.slice().reverse().map((item) => (
                              <div key={item.label} className="flex items-center gap-2">
                                <span className="h-2.5 w-2.5 rounded" style={{ backgroundColor: item.color }} />
                                <span className="text-[0.52rem] font-bold uppercase tracking-[0.1em] text-slate-500">{item.label}</span>
                              </div>
                            ))}
                            <div className="flex items-center gap-2">
                              <span className="h-2.5 w-2.5 rounded bg-[#1f1633]" />
                              <span className="text-[0.52rem] font-bold uppercase tracking-[0.1em] text-slate-500">Sin cobertura</span>
                            </div>
                          </div>
                        </div>
                      ) : (
