const DEFAULT_TEXTS = {
  ...
  eje01_purpose: 'Establecer la música como vehículo de inclusión, identidad and reconciliación, garantizando que todas las personas, sin distinción, puedan vivirla plenamente como parte de su vida, su territorio y su comunidad.',
...
export const saveWebText = (key, content, status = 'publicado', author = 'Webmaster') => {
  try {
    const saved = localStorage.getItem('pnmc_web_texts');
...
export const getWebTextDetails = (key) => {
  try {
    const saved = localStorage.getItem('pnmc_web_texts');
    if (saved) {
      const parsed = JSON.parse(saved);
...
export const getWebTextsKeysList = () => [
  // Barra de navegación (General)
